...
// Deterministic amenity/capacity fill-ins so seeded listings support
// advanced filtering (price range, guests, bedrooms, amenities).
const AMENITY_POOL = {
  Beach:         ["wifi", "ac", "pool", "kitchen", "parking"],
  "Iconic City": ["wifi", "ac", "kitchen", "workspace", "tv"],
  Mountain:      ["wifi", "heating", "kitchen", "parking", "tv"],
  "Amazing Views":["wifi", "kitchen", "parking", "workspace"],
  Pool:          ["wifi", "ac", "pool", "kitchen", "gym", "parking"],
  Castles:       ["wifi", "heating", "kitchen", "tv", "breakfast"],
  Farms:         ["wifi", "kitchen", "parking", "pets", "breakfast"],
  Room:          ["wifi", "ac", "tv", "workspace"],
  Trending:      ["wifi", "ac", "pool", "kitchen", "tv", "parking"]
};

function deriveCapacity(price) {
  if (price >= 4000) return { maxGuests: 10, bedrooms: 4, beds: 6, bathrooms: 4 };
  if (price >= 2500) return { maxGuests: 8, bedrooms: 3, beds: 4, bathrooms: 3 };
  if (price >= 1500) return { maxGuests: 6, bedrooms: 2, beds: 3, bathrooms: 2 };
  if (price >= 1000) return { maxGuests: 4, bedrooms: 2, beds: 2, bathrooms: 1 };
  return { maxGuests: 2, bedrooms: 1, beds: 1, bathrooms: 1 };
}

const enriched = sampleListings.map((l) => {
  const cap = deriveCapacity(l.price);
  const amenities = AMENITY_POOL[l.category] || AMENITY_POOL.Trending;
  return {
    ...l,
    ...cap,
    amenities,
    cleaningFee: Math.round(l.price * 0.08),
    weeklyDiscount: l.price >= 2000 ? 10 : 5,
    monthlyDiscount: l.price >= 2000 ? 20 : 15,
    minStay: 1,
    isAvailable: true
  };
});

module.exports = { data: enriched };
